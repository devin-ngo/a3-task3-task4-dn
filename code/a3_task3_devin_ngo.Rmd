...
urchins_summary <- urchins %>% 
  ...
  summarize (mean_size = round(mean(size),4),
             median_size = round(median(size),4),
             sd_size = round(sd(size),4),
             n = n()) %>% 
...
Takeaways: Based on the graph and table above, the red urchin seems to display a normal distribution while the purple urchin seems to possibly be skewed in the positive direction. The red sea urchin has a much larger mean and median size, while also having a larger standard deviation. The difference between the two means is 2.307, giving a percent difference of \~41%, which is large.
...
This line graph reflects changes in the purple urchin population at different reefs where there has been a decline over the years, including a sharp decline from 2011 to 2013 and from 2019 to 2020. To determine whether the changes in counts by year are reflective of actual changes,
